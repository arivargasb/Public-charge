...
var json = "data/clean/WIC_state_clean.json";
...
for (var i = 0; i < changes.length; i++) {
  ...
  array_0618.push({
    ...
    "color": getColor(changes[i].wic_chan),
    "change": Number(changes[i].wic_chan).toFixed(1)
      });
    }
       ...
       layer_0918.push(L.geoJson(geodata[j], {
         style: function(feature) {
           return {
             ...
             + "</h3> <hr> <h4>Change in WIC: " +array_0918[i].change 
             + "%</h4>")
             );
             };
           };
       ...
       layer_1218.push(L.geoJson(geodata[j], {
         style: function(feature) {
           return {
             ...
             + "</h3> <hr> <h4>Change in WIC: " +array_1218[i].change 
             + "%</h4>")
             );
             };
           };
       ...
       layer_0319.push(L.geoJson(geodata[j], {
         style: function(feature) {
           return {
             ...
             + "</h3> <hr> <h4>Change in WIC: " +array_0319[i].change 
             + "%</h4>")
             );
             };
           };
       ...
       layer_0619.push(L.geoJson(geodata[j], {
         style: function(feature) {
           return {
             ...
             + "</h3> <hr> <h4>Change in WIC: " +array_0619[i].change 
             + "%</h4>")
             );
             };
           };
...
var WIC_1218 = L.layerGroup(layer_1218);
var WIC_0319 = L.layerGroup(layer_0319);
var WIC_0619 = L.layerGroup(layer_0619);
...
var div = L.DomUtil.create('div', 'info legend'),
WIC = [-11, -7.5, -5 , -2.5, -1, -0.5,  0],
  labels = [];
  for (var i = 0; i < WIC.length; i++) {
  WIC[i] = WIC[i].toFixed(1); 
  ...
  for (var i = 0; i < WIC.length; i++) {
    div.innerHTML += 
    '<i style="background-color:' + getColor(WIC[i]) + ';">&nbsp&nbsp&nbsp;</i> ' +
      WIC[i] + (WIC[i + 1] ? '&nbsp&nbsp-&nbsp&nbsp' + (WIC[i + 1] -.1) + '<br>' : '+<br><br>');
    }
