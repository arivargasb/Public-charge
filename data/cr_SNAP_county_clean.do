...
global save_st_ben		"stats/SNAP_state_tot"	
global save_scatter		"charts/SNAP_undocumented_"	
global save_wide		"clean/snap_state"
/****************************************
 1. Import SNAP datasets and clean
****************************************/
...
snap_beneficiaries pop_un* sem semester pop_co pop_st
...
i( county_state_id state_name county_name pop_hispan_prop pop_un_co_per pop_un_st_per state_id county_id ) j(semester) string

preserve
collapse (sum) snap_beneficiaries* (max) pop_st , by(state_name)

forval x = 15/17 {
	gen snap_change_07`x' = ((snap_beneficiaries_07`x'/snap_beneficiaries_01`x')-1)*100
	label var snap_change_07`x' "SNAP change in beneficiaries 07`x'"
	local y=`x'+1
	gen snap_change_01`y' = ((snap_beneficiaries_01`y'/snap_beneficiaries_07`y')-1)*100
	label var snap_change_01`y' "SNAP change in beneficiaries 01`y'"
}

gen snap_change_0718 = ((snap_beneficiaries_0718/snap_beneficiaries_0118)-1)*100
label var snap_change_0718 "SNAP change in beneficiaries 0718"
gen snap_change_0119 = ((snap_beneficiaries_0119/snap_beneficiaries_0718)-1)*100
label var snap_change_0119 "SNAP change in beneficiaries 0119"
keep state_name snap_change* pop_st
save "${save_wide}", replace

restore

// gen snap_chan_0118_0119 = ((snap_beneficiaries_0119/snap_beneficiaries_0118)-1)*100
// gen snap_chan_0718_0119 = ((snap_beneficiaries_0119/snap_beneficiaries_0718)-1)*100


// collapse (sum) snap_beneficiaries* , by(state_name)

// forval x = 15/18 {
// 	gen snap_change07`x' = ((snap_beneficiaries07`x'/snap_beneficiaries01`x')-1)*100
// 	local y=`x'+1
// 	gen snap_change01`y' = ((snap_beneficiaries01`y'/snap_beneficiaries07`x')-1)*100
// }

//
// reshape long snap_beneficiaries snap_change, i(state_name ) j(semester) string
//
// collapse (sum)  snap_beneficiaries snap_change, by(state_name semester)
//
// gen month=1 if semester== "0115" | semester== "0116" | semester== "0117" | semester== "0118" | semester== "0119"
// replace month=7 if semester== "0715" | semester== "0716" | semester== "0717" | semester== "0718"
//
...
// replace year=2019 if semester=="0119" | semester=="0719"
//
// gen sem="1" if month==1
// replace sem="2" if month==7
// drop semester
// tostring year, replace
// gen semester=sem+"-"+ year
//
// collapse (sum) snap_benef (mean) snap_change, by(semester state_name)
// drop if state_name=="" | snap_beneficiaries==0
// preserve
// collapse (sum) snap_benef (mean) snap_change, by(semester)
// gen state_name="US"
// tempfile us
// save `us', replace
// restore
// append using `us'
// drop if snap_change==0
// keep if state_name=="California" | state_name=="US" | state_name=="Texas" | ///
// state_name=="Florida" | state_name=="Illinois"
// export delimited using "/Users/ariadna/Box Sync/public charge blog/data/clean/SNAP_state_clean.csv", replace
// // Calculate biannual change in SNAP enrollment
// */
...
foreach x of local sem {
	...
	reg snap_biannual_chan_`x' pop_un_co_per if  pop_un_co_per!=. 
	est store reg`x'
	}
...
local sem "0715 0116 0716 0117 0717 0118 0718 0119"
foreach x of local sem {
	scatter snap_biannual_chan_`x' pop_un_co_per if pop_un_co_per!=. || /// 
	lfit snap_biannual_chan_`x' pop_un_co_per   if pop_un_co_per!=.
	graph save "${save_scatter}`x'", replace
	}
